// ============================================================
// Ejercicio Adicional 2: Detector de Movimiento y Clasificacion
// de Orientacion (sobre el codigo del Ejercicio 3)
...
bool inicializarMPU6050() {
  ...
  // 3. Asegurar que PWR_MGMT_2 (0x6C) este en 0x00 (Todos los ejes encendidos)
...
// --- Ejercicio Adicional 2: movimiento y orientacion ---

String clasificarOrientacion(float pitch, float roll, float azG) {
  if (azG < -0.8f)                                return "INVERTIDO";
  if (fabsf(pitch) >= 75.0f || fabsf(roll) >= 75.0f) return "VERTICAL";
  if (fabsf(pitch) >= 15.0f || fabsf(roll) >= 15.0f) return "INCLINADO";
  return "PLANO";
}

// Se llama en loop(), despues de calcular axG, ayG, azG, pitch, roll.
void detectarYLoguear(float axG, float ayG, float azG, float pitch, float roll) {
  float magnitud   = sqrtf(axG*axG + ayG*ayG + azG*azG);
  bool  movimiento = (magnitud > 1.3f || magnitud < 0.7f);

  String orientacion = clasificarOrientacion(pitch, roll, azG);
  static String orientacionAnterior = "";

  if (movimiento) {
    Serial.printf("[%8lu ms] MOVIMIENTO detectado | magnitud=%.3f g\r\n",
                  millis(), magnitud);
  }

  if (orientacion != orientacionAnterior) {
    Serial.printf("[%8lu ms] Orientacion: %s | Pitch:%.1f Roll:%.1f\r\n",
                  millis(), orientacion.c_str(), pitch, roll);
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n=== IoT USC - Movimiento y Orientacion ===");
...
void loop() {
  ...
  // --- LECTURA 4: MPU6050 (movimiento y orientacion) ---
  if (!sensorConectado) {
    ...
    if (leerRegistros(MPU6050_ACCEL_XOUT_H, data, 14)) {
      ...
      detectarYLoguear(axG, ayG, azG, pitch, roll);
    } else {
